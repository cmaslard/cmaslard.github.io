...
df <- get_grants() %>%
  mutate(
    sponsor_id = paste0(seq_along(sponsor), " | ", sponsor),
    start_period_hatch = start_period,
    end_period_hatch   = end_period
  ) %>%
  mutate(
    start_period = if_else(
      start_period < today & end_period < today,
      as.Date(NA),
      if_else(start_period < today & end_period > today, today, start_period)
    ),
    label_amount = dplyr::case_when(
      tolower(status) == "funded" & !is.na(amount_awarded) ~
        paste0(scales::comma(amount_awarded, accuracy = 0.01), " ", currency),
      tolower(status) == "not funded" & !is.na(amount_requested_by_applicant) ~
        paste0(scales::comma(amount_requested_by_applicant, accuracy = 0.01), " ", currency),
      TRUE ~ ""
    ),
    status_plot = dplyr::case_when(
      tolower(status) == "funded" ~ "Funded",
      tolower(status) == "in preparation" ~ "In preparation",
      tolower(status) == "under evaluation" ~ "Under evaluation",
      tolower(status) == "submitted" ~ "Submitted",
      tolower(status) == "not funded" ~ "Not funded",
      TRUE ~ "Other"
    ),
    # pour figer l’ordre dans la légende
    status_plot = factor(status_plot,
                         levels = c("Funded", "In preparation", "Under evaluation", "Submitted", "Not funded", "Other"))
  )

# palette pastel
pal <- c(
  "Funded"           = "#B7E4C7",  # vert pastel
  "In preparation"   = "#A9D6E5",  # bleu pastel
  "Under evaluation" = "#FFD6A5",  # orange pastel
  "Submitted"        = "#D0D0D0",  # gris clair
  "Not funded"       = "#FFADAD",  # rouge/rose pastel
  "Other"            = "#E6E6E6"
)

p1 <- ggplot(df) +
  geom_segment(aes(
    x = start_period_hatch, xend = end_period_hatch,
    y = reorder(sponsor_id, as.numeric(start_period_hatch)),
    yend = reorder(sponsor_id, as.numeric(start_period_hatch)),
    color = status_plot
  ), size = 6, alpha = 0.35) +
  geom_segment(aes(
    x = start_period, xend = end_period,
    y = reorder(sponsor_id, as.numeric(start_period_hatch)),
    yend = reorder(sponsor_id, as.numeric(start_period_hatch)),
    color = status_plot
  ), size = 6) +
  geom_text(aes(
    x = start_period_hatch + (end_period_hatch - start_period_hatch) / 2,
    y = reorder(sponsor_id, as.numeric(start_period_hatch)),
    label = label_amount
  ), size = 3, color = "black") +
  theme_minimal() +
  labs(title = "Financing Gantt chart", x = "Period", y = "", color = NULL)+
  theme(
  axis.ticks.y = element_blank(),
...
ggplotly(p1, height = plot_height) %>%
  layout(
    legend = list(
      orientation = "h",
      x = 0, xanchor = "left",
      y = -0.30, yanchor = "top",   # <- plus bas qu'avant
      title = list(text = "")
    ),
    xaxis = list(
      title = list(standoff = 2)    # <- titre "Period" plus proche de l’axe
    ),
    shapes = list(
      list(
        type = "line",
    ...
    margin = list(l = 240, b = 90, t = 30)  # <- b plus grand pour éviter le chevauchement
  )
