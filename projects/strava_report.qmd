...
save(my_data, act_data,stoken, file = "projects/media/strava_report/data_strava.RData")
```
...
```{r}
load(here::here("projects/media/strava_report/data_strava.RData"))
...
act_data<- act_data %>%
  ...
  dplyr::mutate(cumulative_duration_years = cumsum(cumulative_duration)) %>%
  dplyr::ungroup() %>% 
